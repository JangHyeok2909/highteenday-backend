...
> 상태: **해결** (2026-08-14, 미커밋) — 원자 증감으로 교체
...
**①을 채택했다 (2026-08-14).** 유실을 확률적으로 줄이는 게 아니라 없애고, 추가 쿼리도 없다.

```java
// PostRepository — 엔티티의 commentCount++ 를 대체한다
...
- **`CommentService`의 맨 끝에서 호출한다.** `clearAutomatically`가 영속성 컨텍스트를
  비우므로 중간에서 부르면 그 뒤의 `post.getUser()` 지연 로딩이 준영속 상태에서 깨진다.
- **엔티티 값은 같이 맞추지 않는다.** 맞추면 더티 체킹이 낡은 값으로 UPDATE를 한 번 더
  날려 방금 한 원자 증감을 덮어쓴다 — 고치려던 것을 그대로 재현하는 셈이다.
- **감소의 하한은 DB가 건다**(`where p.commentCount > 0`). 애플리케이션에서 읽고 판단하던
  예전 방식(`if (this.commentCount > 0)`)은 그 자체가 갱신 유실의 원인이었다.

엔티티의 증감 메서드는 삭제하고 그 자리에 "다시 만들지 말 것" 주석을 남겼다. 단위 테스트
4개를 엔티티 값 검증에서 **리포지토리 호출 검증**으로 바꿨다 — 엔티티 값을 보는 테스트는
갱신 유실을 원리적으로 잡지 못한다.

**이미 어긋난 70건은 그대로다.** 이 수정은 앞으로의 유실만 막는다. 기존 large는 P0-2
재생성으로 해소한다(일회성 보정 쿼리는 쓰지 않기로 했다 — 어차피 옛 샘플러 세대라 폐기 대상).
...
- **원인이 갱신 유실이라는 것 — 2026-08-14 재현으로 확정.** 추론이 아니다.
  같은 게시글에 동시 댓글 30건을 넣었더니 성공 6건인데 카운터는 3건만 올랐다.

  ```text
  대상 게시글 100755 — 시작 commentCount = 19
  동시 댓글 30건 요청 → 성공 6건
  끝난 뒤 commentCount = 22   (기대 25)
  → 3건 유실
  ```

  이 재현이 **재생성의 선행 조건을 정해 줬다.** 시더는 Zipf로 인기글에 댓글을 몰아넣고
  동시성 10으로 돌리므로 시더 자신이 드리프트를 만든다. 고치지 않고 재생성했다면 새
  데이터셋도 끝나는 순간 이미 어긋나 P0-2 완료 기준을 통과하지 못했을 것이다.
