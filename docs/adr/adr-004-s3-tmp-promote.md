...
- 트레이드오프·한계 (코드로 확인되는 것):
  - 게시글 저장과 S3 복사가 한 트랜잭션 흐름에 있지만 S3는 트랜잭션에 참여하지 않는다. 복사 후 커밋 실패 시 영구 객체가 고아로 남을 수 있으며 [STORAGE-001](../issues/)에서 추적한다.
  - 처음에는 승격 시점에 `tmp/{userId}/` 전체를 지웠지만, 2026-08-28에 이번 요청이 승격한 URL만 지우도록 바꿨다. 그 대가로 승격되지 않은 초안의 tmp 파일이 남으며, 정리는 S3 라이프사이클 규칙의 몫이다.
