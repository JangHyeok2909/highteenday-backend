...
- `RecentHotPost`(게시판별 실시간 인기글의 DB 흔적)도 존재하며 `Post`·`User`를 참조한다 (`domain/hot/RecentHotPost.java`). 현재 핵심 경로는 `DailyHotPost`다 — 흐름은 [crosscutting/schedulers.md](crosscutting/schedulers.md) 참고.
