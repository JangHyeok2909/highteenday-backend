# level-series.ps1 — 레벨 시계열을 위한 야간 반복 실행기
#
# 왜 이 모양인가
# --------------
# 한 세트를 길게 재는 것보다 레벨을 자주 찍는 편이 낫다. 세트 안 CV 가 7.07% 이므로
# 3회면 레벨 추정 표준오차가 약 4% 가 되어, 세트 하나가 하나의 레벨 표본이 된다.
#
# 세트(약 37분)와 유휴(60분)를 번갈아 둔다. 유휴 구간이 있어야 같이 도는
# idle-bench-logger 가 **부하 없는** 환경 표본을 남길 수 있다.
#
# 고아 컨테이너 방어
# ------------------
