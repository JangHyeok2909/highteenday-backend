 ...
 *
 * 환경 속도는 단조 열화가 아니라 **양방향**으로 움직인다(유휴 벤치 single 기준 하루 안에
 * 4,962 → 5,970ms 수준). 그래서 "무엇이 누적되는가"가 아니라 "언제 어느 방향으로
 * 전환하는가"를 봐야 하고, 깊은 계측이 아니라 **촘촘한 시간 해상도**가 필요하다.
 *
 * 다만 이 값이 앱 성능으로 전달되지는 않는다 — 판정용 운용점(RATE=4)에서 잰 4시간 간격
 * 두 세트는 유휴 벤치가 18% 느려지는 동안 앱 p95 가 1.5% 만 움직였다. 이 계열은 앱 수치의
 * 설명 변수가 아니라 **환경 상태의 독립 관측**으로 쓴다.
 *
