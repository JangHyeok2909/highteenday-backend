...
flowchart LR
    ...
    M -->|"예"| B["기준선 자동 선택<br/>실행 조건이 같은 · 정상 측정된 최근 실행"]
    B --> J{rules.json 판정}
    J -->|PASS| OK["✅ 통과"]
...
**기준선을 손으로 저장하지 않는다.** 이력에 쌓인 "실행 조건이 같고 정상적으로 측정된 최근
실행"이 자동으로 기준이 된다. 조건은 시나리오·환경·데이터셋·부하 프로파일·측정 구간 설계
(warmup/measure/rampdown 길이·mode·gatePhase, T-03/S-08)이며, 하나라도 다르면 그건 다른
실험이라 상대 비교를 생략한다(절대 게이트는 계속 적용). 부하 스크립트 지문만 다른 경우는
비교하되 게이트를 연다.

**성능이 나빴다는 이유로는 기준선에서 빼지 않는다(S-10).** 기준선 자격은 측정 무결성
(`UNMEASURED`와 measure 구간 미완 실행은 제외)과 실행 조건의 비교 가능성으로만 정해진다.
threshold 미달은 성능 상태일 뿐이라 자격에 영향을 주지 않는다 — 개선 전후를 비교하려면
느린 Before가 기준선이어야 하기 때문이다. 대신 그 사실은 지우지 않고 리포트에 경고로
표시하며, 절대 게이트가 기준선과 무관하게 계속 돌아 거짓 PASS를 막는다. 자세한 규칙은
[`../PERFORMANCE-MANAGEMENT.md`](../PERFORMANCE-MANAGEMENT.md) §5 "기준선 자격" 참고.
...
| **비교 가능성** | large/200VU 실행이 small/15VU 실행과 비교된다 |

부하 테스트 수치는 본질적으로 노이즈가 있다. "직전보다 나빠졌다"를 그대로 회귀로 부르면
오탐이 쏟아지고 **결국 아무도 결과를 보지 않게 된다(경보 피로).**

앞의 넷은 전부 값의 **크기**를 본다. 다섯 번째만 성격이 다르다 — 두 수치가 **애초에
비교 가능한 실행에서 나왔는지**를 본다. 이 장치가 없던 시절 부하 13배·데이터셋 100배가
다른 실행이 기준선으로 뽑혀 **+59,000% 거짓 회귀**가 보고됐다(T-02). 판정 책임은
`tools/lib/comparability.js` 가 단독으로 갖는다. 조건표와 등급은
[PERFORMANCE-MANAGEMENT.md](../PERFORMANCE-MANAGEMENT.md) 의 "기준선 선택" 절에 있다.
