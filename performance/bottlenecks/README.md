...
### ② Before와 After는 같은 운용점에서 재야 한다

**포화 영역에서 재면 안 된다.** 이용률이 1에 가까워지면 대기가 `1/(1-ρ)`로 발산하므로,
호스트의 작은 변동이 응답시간에서 수십 배로 증폭된다. 그 상태에서는 개선 폭이 측정
노이즈에 묻힌다.

- 판정용 측정은 **open model**로 돌린다 — `-e RATE=<초당 도착률>`. 도착률을 고정해야
  p95가 처리량의 그림자에서 벗어난다
- 운용점은 보정 곡선으로 확인한 **절벽 아래**를 쓴다. 현재 medium 기준 `RATE=4`
  (앱 CPU 37~45%, Hikari 대기 0)
- `tools/lib/saturation.js`가 실행마다 포화를 판정한다. `SATURATED`로 나온 실행은
  개선 근거로 인용하지 않는다

이 조건을 지키면 시간을 두고 비교해도 된다. 코드를 바꾸지 않고 4시간 간격으로 잰 두 세트가
**−1.5%** 차이였다(같은 구간에 유휴 CPU 벤치는 18% 느려졌다). 세트 안 CV는 3.2%다.
