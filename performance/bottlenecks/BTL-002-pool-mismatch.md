...
> 상태: 의심 (풀 확대 방향은 EXP-006 에서 관찰 완료 — 아래 ④)
> 관련: EXP-002, EXP-006
...
> **기록 완료 (2026-08-29, EXP-006).** Before 풀 크기는 **10**(HikariCP 기본값)이다.
> `normal-day` 5회 세트(`reports/repeatability/perf-2026-08-26T00-44-58.json`, 커밋
> `f915176e`)에서 `pool.hikariMax`가 5회 모두 10이었고, 같은 세트의
> `mysql.threadsConnected.max`가 5회 모두 11(풀 10 + exporter 1)이었다.
>
> 풀을 20으로 올린 관찰 결과는 `experiments/EXP-006-pool-sizing/README.md`에 있다.
> 요약: **커넥션 대기와 포화 판정은 사라졌고 응답 p95는 기준선 범위 안에 그대로 있었다.**
> 회수된 커넥션 획득 대기가 0.96ms → 0.95ms 였기 때문이다. 이 병목의 상태를 "의심"에서
> 내리지 않는 이유는 두 가지다. ① EXP-006 은 앱 커밋이 44개 다른 상태에서 측정돼
> 지연·자원 비교가 잠정값이다(EXP-006 9.3절). ② 풀을 **줄이는** 방향은 아직 안 쟀다 —
> "풀이 진짜 병목일 때의 지표"를 같은 조건에서 확보해야 이 문서의 증상 서술이
> 실측으로 뒷받침된다.
